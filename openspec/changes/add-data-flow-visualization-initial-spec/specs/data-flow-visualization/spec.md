...
The implementation SHALL be structured into the following logical modules:
- Parser (DSL text → AST)
- Validator (AST → diagnostics + normalized model)
- Graph builder + bundling (field graph and node-to-node aggregation)
- Layout engine (compute node positions and edge routes)
- Renderer (SVG rendering and hit testing)
- Interaction controller (hover/selection logic, zoom/pan)
- Persistence/share/export utilities (`localStorage`, URL hash, SVG/PNG)
...
UserInput.password -> Database.hashedPassword
```
...
Source.userId -> Transformer.id
Source.email -> Transformer.normalizedEmail
Source.createdAt -> Transformer.date

Transformer.id -> Warehouse.user_id
Transformer.normalizedEmail -> Warehouse.email
Transformer.date -> Warehouse.created_date
```
...
Request.body.password -> AuthService.credentials.pass
AuthService.token -> Response.accessToken
```

## Appendix: PEG grammar sketch (informative)
```peg
Diagram = _ (Node / Connection)* _

Node = "node" _ id:Identifier _ label:Label? _ "{" _ fields:Fields _ "}" _ {
  return { type: 'node', id, label, fields };
}

Connection = source:FieldPath _ "->" _ target:FieldPath _ {
  return { type: 'connection', source, target };
}

Fields = (Field _)*

Field = name:Identifier _ ":" _ type:Type _ nested:NestedFields? {
  return { name, type, nested };
}

NestedFields = "{" _ fields:Fields _ "}"

FieldPath = node:Identifier "." path:Path {
  return { node, path };
}
...
Label = "[" _ "label" _ "=" _ value:String _ "]"
...
## Appendix: Principles and primary user journey (informative)

### Principles
- The DSL text is the single source of truth (diagram is derived from DSL).
- Minimal visual noise: connections are thin and gray in calm state; details appear on hover/selection.
- No default 1:1 “spider web”: field connections are aggregated/bundled between nodes by default.

### Primary user journey
1. The user writes or pastes DSL in the editor (left pane).
2. The diagram renders automatically with live preview (right pane).
3. The user hovers/taps any field to see its path through the diagram.
4. The user zooms/pans and can reset the view with a “Reset view” control.

## Appendix: High-level component diagram (informative)
```text
┌─────────────────────────────────────┐
│         App Container               │
├─────────────────────────────────────┤
...
## Appendix: Suggested delivery phases (informative)

### Phase 1: Parser and AST
1. Define the DSL grammar
2. Implement the parser (PEG-based)
3. Test parsing with multiple diagram examples

### Phase 2: Layout engine
1. Integrate Dagre
2. Compute node positions
3. Generate edge paths (e.g., Bezier curves)

### Phase 3: Renderer
1. Render nodes in SVG
2. Render connections in SVG
3. Apply basic styling

### Phase 4: Interactivity
1. Field hover effects
2. Connection highlighting
3. Zoom/Pan functionality

### Phase 5: UI/UX
1. Integrate a code editor component
2. Live preview
